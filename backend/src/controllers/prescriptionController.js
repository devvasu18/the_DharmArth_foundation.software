...
exports.uploadPrescription = async (req, res) => {
    try {
        ...
        if (req.body.shippingAddress) {
            try {
                parsedShippingAddress = JSON.parse(req.body.shippingAddress);
            } catch (e) {
                console.error("Failed to parse shippingAddress:", e);
            }
        }

        // Validate Pincode if configured
        if (parsedShippingAddress && parsedShippingAddress.zip) {
            const { getPharmacyConfig } = require('./settingsController');
            const config = await getPharmacyConfig();
            if (config.acceptedPincodes && config.acceptedPincodes.trim() !== '') {
                const allowedPincodes = config.acceptedPincodes.split(',').map(pin => pin.trim());
                const userPincode = parsedShippingAddress.zip.trim();
                if (!allowedPincodes.includes(userPincode)) {
                    return res.status(400).json({ 
                        message: `Sorry, we do not deliver to this pin code. Accepted pin codes are: ${allowedPincodes.join(', ')}` 
                    });
                }
            }
        }
        ...
        const prescription = await Prescription.create({
            ...
            pendingShippingAddress: parsedShippingAddress
        });
...
// @desc    Create medicine order manually (Admin)
// @route   POST /api/prescriptions/admin-create
// @access  Private/Admin
exports.createAdminOrder = async (req, res) => {
    try {
        const { userMobile, userName, shippingAddress, verifiedItems, adminNote } = req.body;
        ...
        if (shippingAddress && shippingAddress.zip) {
            ...
            if (config.acceptedPincodes && config.acceptedPincodes.trim() !== '') {
                ...
                if (!allowedPincodes.includes(userPincode)) {
                    return res.status(400).json({ 
                        message: `Sorry, we do not deliver to this pin code. Serviceable pin codes are: ${allowedPincodes.join(', ')}` 
                    });
                }
            }
        }

        // Look up or create guest user
        ...
        const prescription = await Prescription.create({
            user: user._id,
            guestName: userName || user.name,
            guestMobile: userMobile,
            status: 'Verified',
            userApproved: true,
            approvalRequired: false,
            orderSource: 'Created by Medical/Admin',
            ...
            pendingShippingAddress: {
                ...shippingAddress,
                phone: shippingAddress.phone || userMobile
            },
            adminNote: adminNote || 'Order manually created by Medical/Admin'
        });
        ...
        prescription.verificationLog.push({
            status: 'Verified',
            updatedBy: req.user._id,
            note: 'Order manually generated by Medical/Admin'
        });
        await prescription.save();
...
// @desc    Admin lookup user by mobile
// @route   GET /api/prescriptions/user-lookup/:mobile
// @access  Private/Admin
exports.adminLookupUser = async (req, res) => {
    try {
        const { mobile } = req.params;
